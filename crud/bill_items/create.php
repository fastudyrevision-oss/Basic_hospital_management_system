...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$bill_id || !$medicine_id || $quantity <= 0 || $price <= 0) {
        ...
        try {
            $stmt = $pdo->prepare("INSERT INTO bill_items (bill_id, medicine_id, quantity, price) VALUES (?, ?, ?, ?)");
            $stmt->execute([$bill_id, $medicine_id, $quantity, $price]);
...
<head>
    ...
    <title>Add Bill Item - Hospital Management System</title>
    <style>
        ...
        <form method="POST">
            <div class="form-group">
                ...
                <select id="bill_id" name="bill_id" required>
                    ...
                    <option value="<?php echo $bill['id']; ?>">#<?php echo $bill['id']; ?> - <?php echo htmlspecialchars($bill['patient_name']); ?> (Rs. <?php echo number_format($bill['total_amount'], 2); ?>) - <?php echo ucfirst($bill['status']); ?></option>
                    <?php endforeach; ?>
                </select>
            </div>
            ...
            <div class="form-group">
                ...
                <select id="medicine_id" name="medicine_id" required>
                    ...
                    <option value="<?php echo $med['id']; ?>"><?php echo htmlspecialchars($med['product_name']); ?></option>
                    <?php endforeach; ?>
                </select>
            </div>
            ...
            <div class="form-group">
                ...
                <input type="number" id="quantity" name="quantity" min="1" placeholder="e.g., 2" required>
            </div>

            <div class="form-group">
                <label for="price">Price per Unit (Rs.) *</label>
                <input type="number" id="price" name="price" step="0.01" min="0" placeholder="e.g., 200.00" required>
            </div>

            <div>
                <button type="submit" class="btn btn-submit">Add to Bill</button>
