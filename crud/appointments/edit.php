...
$id = $_GET['id'] ?? null;
$appointment = get_by_id('appointments', $id, $pdo);
$error = '';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$doctor_id || !$patient_id || empty($appointment_date)) {
        ...
        try {
            $stmt = $pdo->prepare("UPDATE appointments SET doctor_id = ?, patient_id = ?, appointment_date = ?, notes = ? WHERE id = ?");
            $stmt->execute([$doctor_id, $patient_id, $appointment_date, $notes, $id]);
        ...
        <form method="POST">
            <div class="form-group">
                ...
                <select id="doctor_id" name="doctor_id" required>
                    ...
                    <option value="<?php echo $doc['id']; ?>" <?php echo $doc['id'] === $appointment['doctor_id'] ? 'selected' : ''; ?>><?php echo htmlspecialchars($doc['first_name'] . ' ' . $doc['last_name']); ?></option>
                    <?php endforeach; ?>
                </select>
            </div>
            ...
            <div class="form-group">
                ...
                <select id="patient_id" name="patient_id" required>
                    ...
                    <option value="<?php echo $pat['id']; ?>" <?php echo $pat['id'] === $appointment['patient_id'] ? 'selected' : ''; ?>><?php echo htmlspecialchars($pat['first_name'] . ' ' . $pat['last_name']); ?></option>
                    <?php endforeach; ?>
                </select>
            </div>
            ...
            <div class="form-group">
                ...
                <input type="datetime-local" id="appointment_date" name="appointment_date" value="<?php echo str_replace(' ', 'T', htmlspecialchars($appointment['appointment_date'])); ?>" required>
            </div>

            <div class="form-group">
                <label for="notes">Notes</label>
                <textarea id="notes" name="notes"><?php echo htmlspecialchars($appointment['notes'] ?? ''); ?></textarea>
            </div>
