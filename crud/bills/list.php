...
<body>
    <div class="container">
        <div class="header">
            <h1>💰 Bills</h1>
            <div>
                <a href="create.php" class="btn btn-add">+ New Bill</a>
        ...
        <table>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td><?php echo htmlspecialchars($bill['pat_first'] . ' ' . $bill['pat_last']); ?></td>
                    <td><?php echo 'Rs. ' . number_format($bill['total_amount'], 2); ?></td>
                    <td><?php echo htmlspecialchars($bill['payment_method'] ?? 'N/A'); ?></td>
                    <td><span class="status status-<?php echo htmlspecialchars($bill['status']); ?>"><?php echo ucfirst(htmlspecialchars($bill['status'])); ?></span></td>
                    <td><?php echo htmlspecialchars($bill['created_at']); ?></td>
                    <td class="actions">
                        <a href="edit.php?id=<?php echo $bill['id']; ?>" class="btn btn-edit">Edit</a>
                        <a href="delete.php?id=<?php echo $bill['id']; ?>" class="btn btn-delete" onclick="return confirm('Are you sure?')">Delete</a>
                    </td>
                </tr>
                <?php endforeach; ?>
            </tbody>
        ...
        <?php if (empty($bills)): ?>
            <p style="text-align: center; margin-top: 20px; color: #666;">No bills found. <a href="create.php">Add one now</a></p>
        <?php endif; ?>
    </div>
</body>
</html>
